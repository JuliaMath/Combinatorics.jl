...
struct SkewDiagram
    ...
    function SkewDiagram(λ::Partition, μ::Partition)
        ...
        n > m && throw(
            ArgumentError("Cannot construct skew diagram with partition lengths $m, $n")
        )
        new(λ, μ)
    end
end
...
function isrimhook(ξ::SkewDiagram)
    ...
    for k in 1:length(youngdiagramList)
        i, j = youngdiagramList[k]
        numNeighbors = 0
        for kp in 1:length(youngdiagramList)
